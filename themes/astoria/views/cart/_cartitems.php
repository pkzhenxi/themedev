...
<div id="genericcart">
    <div class="row-fluid labels-cart">
        <div class="span6 label-desc"><span class="cart_header"><?= Yii::t('cart','Description'); ?></span></div>
        <div class="span3 label-price rightitem"><span class="cart_header"><?= Yii::t('cart','Price'); ?></span></div>
        <div class="span1 centeritem label-qty"><span class="cart_header"><?= Yii::t('cart','Qty'); ?></span></div>
        <div class="span2 label-total rightitem"><span class="cart_header"><?= Yii::t('cart','Total'); ?></span></div>
	</div>
	    ...
	    <div class="row-fluid total-area remove-bottom">
		    ...
		    <div id="cartTaxes" class="total-area">
			    <?php echo $this->renderPartial('/cart/_carttaxes',array('model'=>$model),true); ?>
		    </div>

		<div class="row-fluid total-area cart-subtotal remove-bottom">
		...
		<div class="row-fluid total-area cart-shipping remove-bottom">
		        <div class="span2 offset8 cart_label"><?= Yii::t('cart',"Total"); ?></div>
		        <div class="span2 cart_price"><span id="cartTotal"><?= _xls_currency($model->total); ?></span></div>
		</div>
		<?php if($model->PromoCode): ?>
			<div class="row-fluid total-area remove-bottom">
			     <div class="span4 offset8 promoCode"><span><?= Yii::t('cart',"Promo Code {code} Applied",array('{code}'=>"<strong>".$model->PromoCode."</strong>")); ?></span></div>
			</div>
		<?php endif; ?>
	...
	<?php if (isset($this->intEditMode) && $this->intEditMode): ?>
		<div class="row-fluid">
			<div class="span12 errorMessage">
				<?php echo Yii::t('cart','<p class="error_msg">Note: Change quantity to zero to remove an item from your cart.</p>'); ?>
			</div>
		</div>
	<?php endif; ?>
</div>
